...
// If archive is empty, uses the most recent archive from the repository.
func (b *Borg) Extract(repo, archive, targetDir string) error {
	if archive == "" {
		archives, err := b.List(repo)
		if err != nil {
			return fmt.Errorf("listing archives to find latest: %w", err)
		}
		if len(archives) == 0 {
			return fmt.Errorf("no archives found in repository %s", repo)
		}
		archive = archives[len(archives)-1].Name
	}

	// borg extract restores paths relative to cwd, so we set Dir to /
	env := b.borgEnv(repo)
	_, err := b.runner.RunWithEnvAndDir(env, "/",
