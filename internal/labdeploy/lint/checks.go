...
import (
	...
	"github.com/jdillenberger/arastack/internal/labdeploy/image"
...
func (l *Linter) buildDummyValues(meta *template.AppMeta) map[string]string {
...
func (l *Linter) lintCompose(name string, meta *template.AppMeta, dummyValues map[string]string) []Finding {
	var findings []Finding

	renderer := template.NewRenderer(l.registry)
	...
	for svcName, svc := range compose.Services {
		...
		if svc.Build == nil && svc.Image != "" {
			ref, err := image.ParseRef(svc.Image)
			if err == nil {
				if ref.IsFloating() {
					findings = append(findings, Finding{
						Template: name,
...
func filterSuppressed(findings []Finding, ignores []string) []Finding {
	var kept []Finding
...
func isSuppressed(f Finding, ignores []string) bool {
