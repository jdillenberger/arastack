...
func (h *Handler) resolveCACertPath() string {
	if h.cfg.CA.CertPath != "" {
		return h.cfg.CA.CertPath
	}
	return filepath.Join(h.ldc.DataDir, "traefik", "certs", "ca.crt")
}

func (h *Handler) caIPBaseURL() string {
	ip := netutil.DetectLocalIP()
	if ip == "" {
		ip = h.ldc.Hostname + "." + h.ldc.Network.Domain
	}
	return "http://" + ip + ":" + strconv.Itoa(h.cfg.Server.Port)
}
