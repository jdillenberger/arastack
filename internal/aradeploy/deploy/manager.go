...
func (m *Manager) Deploy(appName string, opts DeployOptions) error {
	...
	if err := SaveSecrets(dataDir, mergedValues, autoGenerated); err != nil {
	...
	if keepData {
		...
		if _, err := os.Stat(secretsPath); os.IsNotExist(err) {
			if info, err := m.GetDeployedInfo(appName); err == nil {
				if tmplMeta, ok := m.registry.Get(appName); ok {
					if err := SaveSecretsFromState(dataDir, info, tmplMeta); err != nil {
