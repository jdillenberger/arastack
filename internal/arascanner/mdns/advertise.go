...
const ServiceType = "_arascanner._tcp"

// Advertise registers the arascanner service via mDNS and returns a shutdown function.
func Advertise(hostname string, port int, version, role string, tags map[string]string) (shutdown func(), err error) {
	txt := []string{
		"hostname=" + hostname,
		"version=" + version,
		"role=" + role,
