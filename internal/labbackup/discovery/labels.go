...
// parseComposeLabels reads a docker-compose.yml file and extracts labbackup.* labels.
...
func extractLabels(raw interface{}) map[string]string {
	...
	case map[string]interface{}:
		for key, val := range v {
			if !strings.HasPrefix(key, "labbackup.") {
				continue
			}
...
// parseBackupLabels converts raw labbackup.* labels into a BackupLabels struct.
func parseBackupLabels(labels map[string]string) BackupLabels {
	bl := BackupLabels{}

	if v, ok := labels["labbackup.enable"]; ok {
		bl.Enable = strings.EqualFold(v, "true")
	}

	// Borg settings
	if v, ok := labels["labbackup.borg.paths"]; ok {
		bl.BorgPaths = v
	}

	// Dump settings
	if v, ok := labels["labbackup.dump.driver"]; ok {
		bl.DumpDriver = v
	}
	if v, ok := labels["labbackup.dump.user"]; ok {
		bl.DumpUser = v
	}
	if v, ok := labels["labbackup.dump.password-env"]; ok {
		bl.DumpPasswordEnv = v
	}
	if v, ok := labels["labbackup.dump.database"]; ok {
		bl.DumpDatabase = v
	}
	if v, ok := labels["labbackup.dump.command"]; ok {
		bl.DumpCommand = v
	}
	if v, ok := labels["labbackup.dump.restore-command"]; ok {
		bl.DumpRestoreCommand = v
	}
	if v, ok := labels["labbackup.dump.file-ext"]; ok {
		bl.DumpFileExt = v
	}

	// Retention overrides
	if v, ok := labels["labbackup.retention.keep-daily"]; ok {
		bl.RetentionKeepDaily = v
	}
	if v, ok := labels["labbackup.retention.keep-weekly"]; ok {
		bl.RetentionKeepWeekly = v
	}
	if v, ok := labels["labbackup.retention.keep-monthly"]; ok {
