...
func init() {
	runCmd.Flags().StringVarP(&interval, "interval", "i", "", "poll interval (default: 30s, env: TRAEFIK_MDNS_INTERVAL)")
...
var runCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		fmt.Printf("traefik-mdns running (runtime: %s, interval: %s)\n", runtime, pollInterval)
...
func resolveInterval() time.Duration {
	s := interval
	if s == "" {
		s = os.Getenv("TRAEFIK_MDNS_INTERVAL")
	}
	if s == "" {
