...
func CheckAll(dataDir string) []CheckResult {
	deps := Dependencies()
...
func CheckNSSwitchMDNS() CheckResult {
	result := CheckResult{Name: "nsswitch-mdns"}
...
func CheckAvahiInterfaces() CheckResult {
	result := CheckResult{Name: "avahi-interfaces"}
...
func CheckResolvedMDNS() CheckResult {
	result := CheckResult{Name: "resolved-mdns"}
...
func CheckAvahiHostnameConflict() CheckResult {
	result := CheckResult{Name: "avahi-hostname-conflict"}
...
// CheckServiceRunning checks if peer-scanner systemd service is active.
func CheckServiceRunning() CheckResult {
	result := CheckResult{
		Name: "peer-scanner-running",
	}
...
func Fix(result CheckResult, dataDir string) error {
	if result.Installed {
		return nil
	}
	...
	case "peer-scanner-running":
		fmt.Println("    Run: labmanager setup peer-scanner")
		return nil
	}
...
func fixNSSwitchMDNS() error {
	...
	if nssModified {
		tmpFile, err := os.CreateTemp("", "nsswitch.conf.peer-scanner.*")
		if err != nil {
	...
	if !strings.Contains(string(allowData), ".local") {
		tmpFile, err := os.CreateTemp("", "mdns.allow.peer-scanner.*")
		if err != nil {
			return fmt.Errorf("creating temp file: %w", err)
		}
		tmpPath := tmpFile.Name()
...
func fixAvahiInterfaces() error {
	...
	tmpFile, err := os.CreateTemp("", "avahi-daemon.conf.peer-scanner.*")
	if err != nil {
		return fmt.Errorf("creating temp file: %w", err)
	}
	tmpPath := tmpFile.Name()
...
func fixResolvedMDNS() error {
	...
	dropInFile := dropInDir + "/peer-scanner.conf"
