...
func fixNSSwitchMDNS() error {
	...
	if nssModified {
		tmpFile, err := os.CreateTemp("", "nsswitch.conf.aramdns.*")
		if err != nil {
	...
	if !strings.Contains(string(allowData), ".local") {
		tmpFile, err := os.CreateTemp("", "mdns.allow.aramdns.*")
		if err != nil {
			return fmt.Errorf("creating temp file: %w", err)
		}
		tmpPath := tmpFile.Name()
...
func fixAvahiInterfaces() error {
	...
	tmpFile, err := os.CreateTemp("", "avahi-daemon.conf.aramdns.*")
	if err != nil {
		return fmt.Errorf("creating temp file: %w", err)
	}
	tmpPath := tmpFile.Name()
...
func fixResolvedMDNS() error {
	...
	dropInFile := dropInDir + "/aramdns.conf"
