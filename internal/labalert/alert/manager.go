...
// Event represents a pushed event from external sources (labdeploy, labbackup, etc.).
type Event struct {
...
// Manager evaluates alert rules and dispatches notifications via labnotify.
type Manager struct {
	store            *Store
...
func (m *Manager) evaluateRule(rule Rule, results []health.Result) {
	...
	case RuleTypeBackupFailed, RuleTypeUpdateFailed:
		// These are handled via HandleEvent (pushed by labbackup/labdeploy), not health checks.
	}
}
...
// SendTest sends a test alert via labnotify.
func (m *Manager) SendTest() error {
	a := Alert{
		ID:        uuid.New().String(),
		Type:      "test",
		Severity:  "info",
		Message:   "Test alert from labalert",
...
func (m *Manager) sendNotification(a Alert, channels []string) {
	...
	if err := m.client.Send(ctx, n); err != nil {
		slog.Error("failed to send notification via labnotify", "error", err)
	}
}
