...
func (srv *Server) handleHeartbeat(w http.ResponseWriter, r *http.Request) {
	...
	// Sanitize the sender's source: trust our own records over what they claim.
	if existing, ok := srv.store.Get(req.Sender.Hostname); ok {
		req.Sender.Source = existing.Source
	} else if r.Header.Get("Authorization") != "" {
		req.Sender.Source = peer.SourceInvite
	} else {
		req.Sender.Source = peer.SourceMDNS
	}
