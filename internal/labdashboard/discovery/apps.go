...
// DeployedRoute holds per-app routing state (mirrors labdeploy's struct).
...
// DeployedApp represents a deployed application (mirrors labdeploy's state file).
...
// It reads labdeploy's config to find the apps_dir, then scans for .labdeploy.yaml state files.
func ListApps(labdeployConfigPath string) ([]string, error) {
	ldc, err := config.ReadLabdeployConfig(labdeployConfigPath)
	if err != nil {
		return nil, err
	}
...
func listAppsInDir(appsDir string) ([]string, error) {
	...
	for _, entry := range entries {
		...
		stateFile := filepath.Join(appsDir, entry.Name(), ".labdeploy.yaml")
...
func GetApp(appsDir, appName string) (*DeployedApp, error) {
	stateFile := filepath.Join(appsDir, appName, ".labdeploy.yaml")
