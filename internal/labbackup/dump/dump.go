...
func (d *Dumper) Restore(app *discovery.App, svc discovery.ServiceBackupConfig, dumpFile string) error {
	...
	// Use shell to pipe dump file into the restore command inside the container
	// cat <dump-file> | docker exec -i <container> <restore-command>
	shellCmd := fmt.Sprintf("cat %s | docker exec -i %s %s",
		dumpFile, containerName, strings.Join(restoreCmd, " "))

	_, err = d.runner.Run("sh", "-c", shellCmd)
	if err != nil {
