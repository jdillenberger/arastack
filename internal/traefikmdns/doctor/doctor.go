...
import (
	...
	"github.com/jdillenberger/arastack/internal/traefikmdns/avahi"
)
...
func CheckAvahiInterfaces() CheckResult {
	...
	ifaces, _ := avahi.PhysicalInterfaces()
...
func fixNSSwitchMDNS() error {
	...
	if nssModified {
		tmpFile, err := os.CreateTemp("", "nsswitch.conf.traefik-mdns.*")
		if err != nil {
	...
	if !strings.Contains(string(allowData), ".local") {
		tmpFile, err := os.CreateTemp("", "mdns.allow.traefik-mdns.*")
		if err != nil {
			return fmt.Errorf("creating temp file: %w", err)
		}
		tmpPath := tmpFile.Name()
...
func fixAvahiInterfaces() error {
	ifaces, err := avahi.PhysicalInterfaces()
	...
	tmpFile, err := os.CreateTemp("", "avahi-daemon.conf.traefik-mdns.*")
	if err != nil {
		return fmt.Errorf("creating temp file: %w", err)
	}
	tmpPath := tmpFile.Name()
...
func fixResolvedMDNS() error {
	...
	dropInFile := dropInDir + "/traefik-mdns.conf"
